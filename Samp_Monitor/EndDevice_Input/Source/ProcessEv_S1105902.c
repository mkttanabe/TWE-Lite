...
static void vProcessS1105902(teEvent eEvent);
static uint8 u8sns_cmplt = 0;

static tsSnsObj sSnsObj;
static tsObjData_S1105902 sObjS1105902;

enum {
	E_SNS_ADC_CMP_MASK = 1,
	E_SNS_S1105902_CMP = 2,
	E_SNS_ALL_CMP = 3
};
...
PRSEV_HANDLER_DEF(E_STATE_IDLE, tsEvent *pEv, teEvent eEvent, uint32 u32evarg) {
	if (eEvent == E_EVENT_START_UP) {
		...
		// S1105902 の初期化
		vS1105902_Init( &sObjS1105902, &sSnsObj );
		vSnsObj_Process(&sSnsObj, E_ORDER_KICK);
		if (bSnsObj_isComplete(&sSnsObj)) {
			// 即座に完了した時はセンサーが接続されていない、通信エラー等
			u8sns_cmplt |= E_SNS_S1105902_CMP;
			V_PRINTF(LB "*** S1105902 comm err?");
...
PRSEV_HANDLER_DEF(E_STATE_RUNNING, tsEvent *pEv, teEvent eEvent, uint32 u32evarg) {
	...
	if ((eEvent == E_EVENT_START_UP) && (u32evarg & EVARG_START_UP_WAKEUP_RAMHOLD_MASK)) {
		V_PRINTF("#");
		vProcessS1105902(E_EVENT_START_UP);
	}
...
PRSEV_HANDLER_DEF(E_STATE_APP_WAIT_TX, tsEvent *pEv, teEvent eEvent, uint32 u32evarg) {
	if (eEvent == E_EVENT_NEW_STATE) {
		...
		S_OCTET(PKT_ID_S1105902); // パケット識別子

		S_OCTET(sAppData.sSns.u8Batt);
		S_BE_WORD(sAppData.sSns.u16Adc1);
		S_BE_WORD(sAppData.sSns.u16Adc2);
		S_BE_WORD(sObjS1105902.au16Result[S1105902_IDX_R]);
		S_BE_WORD(sObjS1105902.au16Result[S1105902_IDX_G]);
		S_BE_WORD(sObjS1105902.au16Result[S1105902_IDX_B]);
		S_BE_WORD(sObjS1105902.au16Result[S1105902_IDX_I]);
...
static void cbAppToCoNet_vHwEvent(uint32 u32DeviceId, uint32 u32ItemBitmap) {
	switch (u32DeviceId) {
	case E_AHI_DEVICE_TICK_TIMER:
		vProcessS1105902(E_EVENT_TICK_TIMER);
		break;
...
static void vProcessS1105902(teEvent eEvent) {
	...
	if (bSnsObj_isComplete(&sSnsObj)) {
		...
		V_PRINTF(LB"!S1105902: R : %d, G : %d, B : %d, I : %d",
			sObjS1105902.au16Result[S1105902_IDX_R],
			sObjS1105902.au16Result[S1105902_IDX_G],
			sObjS1105902.au16Result[S1105902_IDX_B],
			sObjS1105902.au16Result[S1105902_IDX_I]
		);
